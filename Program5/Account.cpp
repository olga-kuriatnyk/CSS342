...
{
	funds[fund_id].depositAmount(amount);
}
...
{
	if (fund_id == MONEY_MARKET || fund_id == PRIME_MONEY_MARKET || fund_id == LONG_TERM_BOND || fund_id == SHORT_TERM_BOND)
	{
		if (fund_id == MONEY_MARKET || fund_id == PRIME_MONEY_MARKET)
		{
			if (funds[fund_id].balanceCheck(amount))
			{
				funds[fund_id].withdrawAmount(amount);
				funds[fund_id].recordTransaction(trns);
				return true;
			}
			...
			else // two funds together have not enough money to withdraw
			{
				if (fund_id == MONEY_MARKET)
				{
					//withdrawFromSimilarAccountsFaild(MONEY_MARKET, amount);
					faild_withdraw(MONEY_MARKET, amount);
				}
				else
				{
					//withdrawFromSimilarAccountsFaild(PRIME_MONEY_MARKET, amount);
		...
		else if (fund_id == LONG_TERM_BOND || fund_id == SHORT_TERM_BOND)
		{
			...
			else // two funds together have not enough money to withdraw
			{
				if (fund_id == LONG_TERM_BOND)
				{
					//withdrawFromSimilarAccountsFaild(LONG_TERM_BOND, amount);
					faild_withdraw(LONG_TERM_BOND, amount);
				}
				else
				{
					//withdrawFromSimilarAccountsFaild(SHORT_TERM_BOND, amount);
...
void Account::withdrawFromSimilarAccoutns(int primary_fund, int secondary_fund, int amount) //TODO: implement
{
	...
	funds[primary_fund].withdrawAmount(available_balance);
	...
	funds[secondary_fund].withdrawAmount(amount);
	Transaction trns2('W', getAccountID(), secondary_fund, amount);
	funds[secondary_fund].recordTransaction(trns2);
}
...
void Account::faild_withdraw(int fund, int amount)
{
	...
	funds[fund].recordTransaction(trns);
}
